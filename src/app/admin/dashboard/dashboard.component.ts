...
export class AdminDashboardComponent implements AfterViewInit {
  ...
  fetchPumpData() {
    this.pumpService.getPumpData().subscribe((data) => {
      console.log('Données des pompes du jour reçues:', data);
      if (data) {
        this.pumpData = {
          dieselLiters: data.diesel?.liters || 0,
          dieselAmount: data.diesel?.amount || 0,
          gazoleLiters: data.gazole?.liters || 0,
          gazoleAmount: data.gazole?.amount || 0
        };
      ...
      console.error('Erreur lors de la récupération des données des pompes du jour:', error);
    });
  }

  fetchHistoricalData(period: string) {
    let observable: Observable<any>;
    if (period === 'lastWeek') {
      observable = this.pumpService.getHistoricalPumpData();
    } else if (period === 'lastMonth') {
      observable = this.pumpService.getMonthlyPumpData(); // Assurez-vous d'avoir cette méthode dans votre service
    } else {
      observable = this.pumpService.getPumpData();
    }

    observable.subscribe((data: HistoricalData) => {
      this.historicalData = {
  ...
  onPeriodChange(event: any) {
    ...
    this.fetchHistoricalData(period);
  }
